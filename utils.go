...
func bpLogger(opts logOptions) {
	w := color.New(color.FgWhite)
	r := color.New(color.FgRed)
	switch opts.Level {
	case "info":
		w.Printf("\033[2K\r[BeatmapPacker] %v Beatmap Pack #%d", opts.Action, opts.PackNum)
	case "infons":
		w.Printf("\033[2K\r[BeatmapPacker] %v Beatmap Pack #%d\n", opts.Action, opts.PackNum)
	case "error":
		w.Printf("\n[BeatmapPacker] ")
		r.Print("(Error) ")
		w.Printf("%v Beatmap Pack #%d:\n %v\n", opts.Action, opts.PackNum, opts.Err)
	}
}
